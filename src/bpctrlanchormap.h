...
public:
  ...
  // ======================================================================
  void SetFRect(FRECT *pRect, LONG left, LONG top, LONG right, LONG bottom) {
    pRect->left = left;
    pRect->top = top;
    pRect->right = right;
    pRect->bottom = bottom;
  ...
  void Initialize(HWND hWndParent, DWORD dwFlags/*=0*/) {
	...
	GetWindowRect(hWndParent, &rcMaxBR);   // Just grabbing a value that should be larger than any controls top left
	rcMaxBR.left = rcMaxBR.right;            // Initialize to something larger than minimum left
	rcMaxBR.top = rcMaxBR.bottom;            // Initialize to something larger than minimum top
    rcMaxBR.right = 0;
    rcMaxBR.bottom = 0;
    ...
    if (dwFlags & ANIF_CALCSIZE) {
      ...
      for (iCtrl=0; iCtrl < m_nCtrlCount; iCtrl++) {
		if (m_Ctrl[iCtrl].rect.left < rcMaxBR.left) rcMaxBR.left = m_Ctrl[iCtrl].rect.left;
		if (m_Ctrl[iCtrl].rect.top < rcMaxBR.top) rcMaxBR.top = m_Ctrl[iCtrl].rect.top;
        if (m_Ctrl[iCtrl].rect.right > rcMaxBR.right) rcMaxBR.right = m_Ctrl[iCtrl].rect.right;
        if (m_Ctrl[iCtrl].rect.bottom > rcMaxBR.bottom) rcMaxBR.bottom = m_Ctrl[iCtrl].rect.bottom;
      
      };
    
      m_rcPrev.right = m_rcPrev.left + rcMaxBR.left + rcMaxBR.right;
      m_rcPrev.bottom = m_rcPrev.top + rcMaxBR.top + rcMaxBR.bottom;
  ...
  void HandleAnchors(RECT *pRectParent) {
    ...
    for (iCtrl=0; iCtrl < m_nCtrlCount; iCtrl++) {
      ...
      if ((pCtrl->nFlags & ANF_DOCK_ALL)==ANF_DOCK_ALL) {
        ...
        SetFRect(&pCtrl->rect, (LONG)0.0, (LONG)0.0, m_rcClient.right, szCtrl.cy);
        bChanged = TRUE;
      ...
      if ((m_uiSizedBorders & ANF_LEFTRIGHT) && (m_szDelta.cx!=0) && (!bChanged)) {
        ...
        switch(pCtrl->nFlags & ANF_LEFTRIGHT) {
          ...
          default               : pCtrl->rect.left+=(LONG)(m_szDelta.cx/2.0);
                                  pCtrl->rect.right=(pCtrl->rect.left+szCtrl.cx);
                                  bChanged = TRUE;
                                  break;
      ...
      if ((m_uiSizedBorders & ANF_TOPBOTTOM) && (m_szDelta.cy!=0)) {
        ...
        switch(pCtrl->nFlags & ANF_TOPBOTTOM) {
            ...
            default             : pCtrl->rect.top+=(LONG)(m_szDelta.cy/2.0);
                                  pCtrl->rect.bottom=(pCtrl->rect.top+szCtrl.cy);
                                  bChanged = TRUE;
                                  break;
        };
  ...
  BOOL EraseBackground(HDC hDC) {
    ...
    while (hWndChild!=NULL) {
      ...
      if ((iCtrl==-1) && (bVisible)) {
		...
		if (_tcsicmp(_T("BUTTON"), szClassName) || 
			(BS_GROUPBOX != (GetWindowLong(hWndChild, GWL_STYLE) & BS_CHECKBOX))) {
